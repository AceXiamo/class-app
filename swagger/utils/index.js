...
// api接口方法存放目录
const API_PATH = path.resolve(__dirname, "../../src/api/app");
...
const tplInsertApi = (apiInfo) => {
    ...
    let allPath = apiInfo.allPath; // 接口地址
    ...
    if (parameters && parameters.length > 0) {
        interfaceName = `${requestName}Data`;
    ...
    if (method.toLocaleLowerCase() === "get") {
        ...
        if (parameters && parameters.length > 0) {
            interfaceParams = `data?: ${interfaceName}`;
            if (allPath.includes('{')) {
                let path = allPath.substring(0, allPath.indexOf('{'))
                let param = allPath.substring(allPath.indexOf('{') + 1, allPath.indexOf('}'))
                allPath = path + "'+ data?." + param + "+'"
                parametersType = ""
            }
        }
        else {
...
const writeFileApi = (fileName, apiData) => {
    ...
    if (apiData[0].post) {
        tplIndex += '/**\n\t* ' + apiData[0].post.tags[0] + '\n */'
    } else {
        tplIndex += '/**\n\t* ' + apiData[0].get.tags[0] + '\n */'
    }
...
const gen = async () => {
    ...
    try {
        ...
        let pathsKeysLen = pathsKeys.length;
        console.log(
            chalk.red(
                "---------------------------------------------------------------"
            )
        ...
        //获取以变量start开头的接口
        pathsKeys = pathsKeys.filter((item) => item.startsWith(start))
        pathsKeysLen = pathsKeys.length
        console.log(
            chalk.blue(start + "的接口数量：") + chalk.yellow(pathsKeysLen)
        );
        console.log(
            chalk.red(
                "---------------------------------------------------------------"
