<template>
  <view class="overflow-auto p-4 bg-repeat-f7 h-full space-y-4 ios-bottom">
    <view @tap="toDetail(item?.id)" v-for="(item, index) in unread" :key="index" class="p-4 w-full bg-white">
    ...
    <view v-if="unread && unread.length > 0" class="text-repeat-99 text-xs w-full text-center pt-8">-以下为未读通知-</view>

    <view @tap="toDetail(item?.id)" v-for="(item, index) in read" :key="index" class="p-4 w-full bg-white">
      <view class="border-0 border-b-[1rpx] border-solid border-repeat-f7 pb-4 relative">
        <view class="font-bold">{{ item?.type == 1 ? '系统通知' : '' }}</view>
