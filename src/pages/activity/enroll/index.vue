<template>
  <view class="h-full overflow-auto flex flex-col">
    <view class="flex mt-[54rpx] font-medium mb-[38rpx]">
      <view class="w-[148rpx] h-1"></view>
      <view class="w-[114rpx] text-center">姓名</view>
      <view class="ml-[66rpx] w-[174rpx] text-center">行业</view>
    </view>
    <view v-for="(item, index) in list" :key="index" class="flex items-center mb-[30rpx]">
      <image
        @tap="toDetail(item?.id)"
        :src="item?.avatar"
        class="w-[68rpx] h-[68rpx] ml-[40rpx] rounded-1/2 border-[1rpx] border-solid border-repeat-79"
      >
      </image>
