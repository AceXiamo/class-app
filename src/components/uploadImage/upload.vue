<template>
	<view>
		<view v-if="showButton" class="flex flex-wrap relative"
			:style="{ 'width': `${size}rpx`, 'height': `${size}rpx` }">
			<view v-for="(url, index) in curUrl" :key="`gallery-${index}`"
				class="relative flex items-center justify-center bg-repeat-ef"
				:style="{ 'width': `${size}rpx`, 'height': `${size}rpx` }">
				<image :src="url" mode="scaleToFill" :style="{ 'width': `${size}rpx`, 'height': `${size}rpx` }"
					@tap="uploadImage" />
			</view>
			<view v-if="curUrl.length < limit">
				<view @tap="uploadImage">
					<slot>
						<view class="bg-repeat-f5 flex flex-col items-center justify-center text-xs"
							:style="{ 'width': `${size}rpx`, 'height': `${size}rpx` }">
...
const curUrl = ref<string[]>(Array.isArray(props.modelValue) ? props.modelValue : (
	props.modelValue ? [props.modelValue] : []
));
watch(() => props.modelValue, (val) => {
	curUrl.value = Array.isArray(val) ? val : (
		val ? [val] : []
	);
});
...
const buttonSizeHStr = computed(() => {
	if (props.cutHeight && props.cutWidth && typeof props.cutHeight === 'number' && typeof props.cutWidth ===
		'number') {
		const size = typeof props.buttonSize === 'number' ? props.buttonSize : parseInt(props.buttonSize, 10);
...
const uploadImage = () => {
	if (props.limit !== -1 &&
		props.limit !== 1 &&
		curUrl.value.length >= props.limit
	) {
		log.showFail('无法继续上传')
		return
	}
		...
		success: (res) => {
			...
			if (res.tempFilePaths.length > 0) {
				...
				if (cropperEnabled.value) {
					...
					fileName.value = 'name' in uploadFile ? uploadFile.name :
						`${tools.generateCode()}.jpg`;
					src.value = res.tempFilePaths[0];
					show.value = true;
					return;
				}
				const files = Array.isArray(res.tempFilePaths) ? res.tempFilePaths : [res
					.tempFilePaths
				];
